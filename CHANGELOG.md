...
- **`--link-profile <name>` flag: profile selection gets its flag tier (ADR-0007 amendment, #19).** The membership resolution chain becomes `--link-profile` > `PI_LINK_PROFILE` > profiles file `default` > none (implicit local: loopback, no token). The flag selects a profile *name* — the facts (`url`, `token`) stay in the profiles file. This closes the residual ambient hole where `PI_LINK_PROFILE` inherits through tmux spawn chains and can silently redirect a terminal to a *valid but unintended* profile (fail-closed only catches unresolvable names); explicit launch commands now carry membership explicitly, matching the fleet launch doctrine. Fail-closed covers the new tier unchanged: an unresolvable flag-named profile is a loud refusal (`profileUnresolved` latch — no dial, no promotion, no auto-reconnect), `/link-connect` retries. An empty/whitespace flag value is a startup error with exit 1, mirroring `--link-name`. Hub-bind token resolution honors the flag (same one-profile resolution). No session entry — membership stays machine-level environment (ADR-0007 §6 unchanged). Env-only and default-only behavior is byte-identical; the new tier only prepends.
