...
- **Compact membership selector: `--link-name` is now `[profile:][workspace/]name` (ADR-0009, #25).** Three membership flags collapse into one string: a `:` splits the profile from the address, the address follows ADR-0008 (`workspace/name` or bare `name`), and each omitted segment falls through its own chain independently (per segment: flag > env). **`--link-workspace` / `--link-profile` and `PI_LINK_WORKSPACE` / `PI_LINK_PROFILE` are deleted** (the #19 flag never shipped — its entry below is absorbed here); `PI_LINK_NAME` returns as the selector's env tier, consumed once so spawned children don't inherit membership. Malformed strings — empty segments, more than one `:` or `/`, reserved characters (name/workspace: `/`, `*`, `:`; profile: `:`) — fail closed with exit 1, the #19 posture. Saved session entries stay granular (`link-name` / `link-workspace` / `link-global`); the profile is still never persisted; `/link-name` runtime rename stays bare-name only. `link_list` gains one fixed hint line for scoped callers (`scoped view: showing home group + global members only`). No wire change (protocol stays v3) and no lockstep rollout — old launch commands fail loudly at the next natural restart.

- **Global-grant badge is plain text (#23).** The globe-emoji badge from ADR-0008/#20 renders as tofu on some terminals, so every surface now uses the plain word: the status bar gets a ` · global` segment, the join banner keeps the bare word `global`, and `link_list` / `/link` suffix the name with ` (global)`. Protocol untouched — wire still carries the boolean grant; this is display-only.

### Breaking

- **Qualified addresses: home workspace × global grant, sessionId takeover, protocol v3 (ADR-0008, #20).** Two orthogonal facts are no longer squeezed into one field. **Home workspace** (workspace segment of the membership string > saved entry > **`default`** — chain as amended by ADR-0009) is where a terminal lives — undeclared now means the `default` workspace (zero-config loopback pairs still find each other), not privilege; the global-observer-by-omission form is retired. **Global grant** (new `--link-global` flag > `PI_LINK_GLOBAL=1` > saved `link-global` entry > false) is the explicit second axis: see everyone, reach everyone, be reachable by everyone — self-declared within the trust domain (the token stays the only security boundary). Both are identity: fixed at startup, persisted, and pre-written by `link_new`. **Address = `workspace/name`**: names are unique per workspace, and every wire `from`/`to` is fully qualified. Bare `to` names in tool calls resolve in the sender's own workspace only (no scope chain — shadowing is dead); `workspace/*` broadcasts one group (a regular may target only its own). `/`, `*` and `:` are reserved characters, rejected loudly at declaration (startup error, exit 1) and at register. The **reach matrix** is one function — same workspace ✓, anyone ↔ global member ✓, regular cross-workspace ✗ — with existence-hiding refusals (identical text whether the target exists or not), cutting every surface (welcome snapshot, membership events, `link_list`, broadcast, status fan-out, direct addressing). **The hub never renames — it accepts or refuses**: `uniqueName` and its silent `-2` suffixes are deleted; `register` carries the pi `sessionId` as identity anchor, so a same-`(workspace, name)` re-register with the same sessionId is a **silent takeover** (old socket closed, no left/joined churn — netsplit heal and resurrection ride this), while a different sessionId is a loud refusal that sets the new **`nameTaken` latch** (authFailed family: no reconnect storm, `/link-connect` retries). A joiner colliding with the hub's own identity is always refused. Protocol **version 3**: register/welcome carry `workspace`/`global`/`sessionId` and echo the effective home + grant (the fail-closed handshake now covers both axes); v2 registers are refused loudly — **the fleet upgrades together**. Display: `link_list` and the panel group by workspace, badge global members with a plain-text `(global)` suffix, and shorten same-workspace addresses (the wire stays qualified). `test/workspace-isolation.mjs` is rewritten for the new model; the other suites speak v3.

- **Launcher execution mode retired: `pi-link` is query-only (ADR-0007 §8, #16).** `pi-link <name>` no longer resolves, resumes, or spawns a session — it exits non-zero with the two explicit recipes. Implicit name→session execution was a hazard both ways: resuming dropped a live agent into whatever context that session last held, and a typo'd name silently created a blank same-named terminal. Launching is explicit now: **fresh start** → `pi --link --link-name <name>`; **resurrect** → `pi-link --resolve <name> -g`, then `pi --link --session <path>`. The CLI keeps only the query modes (`--list`, `--resolve`, `--version`); help/usage and the `--list` footer teach the explicit spellings. The `PI_LINK_NAME` internal env handoff is deleted end-to-end (the launcher was its only producer; the extension's consume-and-delete path and its session-name seeding are gone — name precedence is now `--link-name` > saved `link-name` > session name > random). This also shrinks #10's surface to `--link-name` only.
- **Config-first dial resolution: `PI_LINK_URL` removed (ADR-0007, #15).** The profile is now the dial target: a client dials the resolved profile's `url` (omitted = `ws://127.0.0.1:$PI_LINK_PORT`) carrying that profile's `token` — one resolved profile answers both "where" and "what ticket". The selection chain is now the membership string's profile segment > `default` > none (as amended by ADR-0009; the selector carries a profile *name* and never a URL — the `PI_LINK_URL` env var is no longer read, and ADR-0002's URL-match selection step dies with it). **Machines relying on ambient `PI_LINK_URL` must write a profiles file** with a `default` profile pointing at the hub URL (rollout coordinates with #6). An unknown selected profile (a typo'd selector segment or a dangling `default`) **fails closed**: loud refusal, no dial, no promotion, no auto-reconnect; `/link-connect` retries after the config is fixed (same latch family as authFailed / workspaceRejected / versionRejected). No profiles file / no `default` = loopback unauthenticated — byte-identical zero-config behavior. Hub-machine doctrine: the hub machine's `default` is a loopback profile (`url` omitted) carrying the fleet token; member machines point `default` at the public URL — with config-first resolution, member machines of a remote fleet structurally cannot self-promote. Amends the ADR-0002 entry below; supersedes #7 item 2.

### Added

- **Hub identity on every display surface (#18).** "Which island am I on?" is now one glance: the join banner reads `Joined link as "name" via <profile> → ws://host:port (N online) · workspace "ws"` (zero-config loopback: `via local (implicit) → ws://127.0.0.1:9900`), the hub-start line names its profile, `link_list` and `/link` carry a single `hub: <profile> → <url>` header line (not per-terminal — the whole list shares one hub), and the status bar appends the profile name when one is selected. Purely client-local display (selection chain + resolved URL) — zero wire change, protocol version untouched. Cross-machine config drift (machine A's `default` → hubA, machine B's → hubB) stops being a silent split. The `pi-link --list -g` hub column stays rejected: session files carry identity, not environment (ADR-0007 §6) — an offline scan cannot know where a session would dial.
- **`PI_LINK_PROFILES_FILE`** env overrides the profiles file *path* (test-isolation knob, same category as `PI_LINK_PORT` — it carries a path, not a URL fact). Read once at module load.
...
- **`link_send` `triggerTurn` is now required + idle-target warning (ADR-0003).** The schema drops the `Optional` default of `false` (which silently dropped dispatches to idle receivers: steer delivery lands in the session without waking the LLM, so the message rots until some unrelated event wakes the terminal). `triggerTurn` is now `Type.Boolean` with no default — the sender must choose per message; LLM callers self-heal via a tool-validation error and retry with the field. On a successful direct send (`to` ≠ `"*"`) with `triggerTurn: false`, if the receiver's last-known status is `idle`, the tool result appends ` ⚠ "x" is idle — message stored, not processed; resend with triggerTurn:true or use link_prompt`; busy (`thinking`/`tool`) or unknown status → no warning. Broadcast and `/link-broadcast` keep their passive FYI semantics. Tool description states both delivery semantics explicitly.
- **Public-reachable hub: profile-based shared-token auth (ADR-0002).** The hub can now bind a non-loopback address and authenticate clients via a shared token. Config: `PI_LINK_HOST` (hub bind host, default `127.0.0.1`), `PI_LINK_URL` (client hub URL, default `ws://127.0.0.1:9900`, `ws://`/`wss://`), profile selection via the membership string (ADR-0009). Tokens live in `~/.pi/agent/pi-link.json` (mode 0600, shape `{profiles:{<name>:{url,token}}, default}`) — the **only** secret source (no env-var token override; env is visible in `ps`). Selection: originally env-based, now the membership string's profile segment > `default` > none (loopback, unauthenticated, byte-identical to pre-ADR-0002 — the #15 and ADR-0009 entries amend this one). **Fail-closed**: a non-loopback bind without a resolvable token refuses to start with an explicit reason. Hub auth: `sha256(token)` digests compared via `crypto.timingSafeEqual` (digest both sides — equalizes length), checked before welcome/joined broadcast; rejection notifies locally with the source address; the token itself is never logged. **Auth rejection ≠ hub loss**: rejection stops auto-reconnect (a wrong-token terminal would hammer the hub every 2s) and notifies clearly pointing at the profiles file; hub loss keeps today's retry-with-backoff. **B6 no-self-promotion**: a client whose URL is non-loopback never runs `startHub` (prevents per-machine island split-brain). Plaintext `ws://` + token to non-loopback → one warning per session, not blocked (policy belongs to operators). `register` gains an optional `token` field (old hubs ignore unknown fields). Loopback with no profile = today's behavior, unchanged.

### Fixed

- **Misconfigured hub bind fails once, loudly (#7 item 1).** `startHub`'s fail-closed refusal (non-loopback bind without a resolvable token) now sets a `hubConfigFailed` latch mirroring `authFailed` — one refusal, then wait for `/link-connect`; previously `initialize()` fell through to `scheduleReconnect` and re-refused every 2–5s forever, the noise masking itself.
- **Profiles file loading works under ESM.** `loadProfilesFile` used a bare `require("node:fs")`, which throws in ESM context and was swallowed by the catch — profiles silently never loaded there. Now a regular top-level import.
